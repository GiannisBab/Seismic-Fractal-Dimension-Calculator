...
example_file_path = "example/EVGI"
try:
    with open(example_file_path, "rb") as f:
        example_file_data = f.read()
    st.sidebar.download_button(
        label="Download Example Seismogram",
        data=example_file_data,
        file_name="EVGI",
        mime="application/octet-stream"
    )
except FileNotFoundError:
    st.sidebar.warning("Local example file not found. Make sure `EVGI` is in the `examples/` folder.")

def try_read_seismic(file_buffer):
    try:
        stm = read(file_buffer)
        return 'obspy', stm
    except Exception:
        return None, None
...
uploaded_file = st.file_uploader("Upload seismic file", type=None, help="Supported formats: SAC, MSEED, etc.")
...
if uploaded_file:
            ...
            with st.spinner("Calculating..."):
                try:
                    ...
                    for tr in stream:
                        ...
                        ax.plot(log_scales, fit_line, 'k--', label=f'Fit: D = {fd:.3f}\nR² = {r2:.3f}')
                    ...
                    for idx, result in enumerate(results):
                        with st.expander(f"{result['id']} - FD: {result['fd']:.3f} (R²={result['r2']:.3f})", expanded=True):
                            ...
                            **Trace ID**  
                            `{result['id']}`
                            """)
                            col2.image(plot_buffers[idx].getvalue(), caption=f"Log-Log Plot for {result['id']}")
                            st.download_button(
    ...
    st.info("📂 Please upload a seismic file or download the example from the sidebar.")